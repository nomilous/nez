...
context = {
  ...
  pull: function() {
    var branch, currentBranch, line, origin, parts, path, ref, workDir, _i, _len, _ref;
    _ref = fs.readFileSync("" + (context.root()) + "/.git_root").toString().split('\n');
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      line = _ref[_i];
      if (!(line.length > 0)) {
      ...
      if (branch !== currentBranch) {
        context.spawnSync('git', ["--git-dir=" + workDir + "/.git", "--work-tree=" + workDir, 'checkout', branch.replace('refs/heads/', '')]);
      }
    }
  },
  push: function() {},
  gitGetBranch: function(gitDir) {
    var branch;
    return branch = context.shellSync("cat " + gitDir + "/HEAD").match(/ref: (.*)$/)[1];
  },
