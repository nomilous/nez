...
var Develop, Objective, Realize, basename, coffee, dirname, extname, fs, inflection, mkdirp, tools, uuid, _ref, _ref1,
...
Develop = (function(_super) {
  ...
  Develop.prototype.configure = function(opts, done) {
    ...
    (_base5 = this.opts).autospawn || (_base5.autospawn = true);
    (_base6 = this.opts).autocompile || (_base6.autocompile = true);
    (_base7 = this.opts).autospec || (_base7.autospec = true);
    return done();
  };
  ...
  Develop.prototype.handleChangedSourceFile = function(filename, _arg) {
    var compiled, error, notice, outDir, outFile, source, srcDir, token;
    ...
    try {
      ...
      return fs.writeFileSync(outFile, compiled);
    } catch (_error) {
      error = _error;
      return console.log({
        CC: error
